	...
	def update():
		...
		for r in receive_data:
			if r['msg'] == Event.CLIENT_ASSOCIATION_REQUEST:
				new_player_data = Player(r['src'], model.bm) #リクエスト元のショートアドレスをプレイやIDにする
				model.set_init_data(new_player_data) # モデルデータの初期化
				player_init_data = new_player_data.dumps_player_data()
				net.transmit_data(r['src'], Event.SERVER_ASSOCIATION_RESPONSE, player_init_data)
			if r['msg'] == Event.CLIENT_SEND_GAMEDATA:
				model.update_player_data(r['payload'])
